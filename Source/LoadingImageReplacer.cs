...
namespace ROLoadingImages
{
    [KSPAddon(KSPAddon.Startup.Instantly, true)]
    class LoadingImageReplacer : MonoBehaviour
    {
        private const string ImagesDir = @"GameData/ROLoadingImages/PluginData/Screens";
        ...
        protected void Update()
        {
            ...
            {   
                ...
                Debug.Log("[ROLoadingImages] Replacing loading screens.");

                List<Texture2D> textures = new List<Texture2D>();

                DirectoryInfo di = new DirectoryInfo(KSPUtil.ApplicationRootPath + ImagesDir);
                foreach (FileInfo fi in di.GetFiles())
                {
                    if (fi.FullName.EndsWith(".dds", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            Debug.LogError($"[ROLoadingImages] Exception loading {fi.FullName}:\n{ex}");
                        }
                    }
                }
                ...
                if (tC > 0)
                {
                    ...
                    Debug.Log($"[ROLoadingImages] Loading screens replaced in {sw.ElapsedMilliseconds} ms.");
                }
                else
                {
                    Debug.LogError("[ROLoadingImages] No screens found in ROLoadingImages/PluginData/Screens!");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[ROLoadingImages] Patching failed: with error {error}, exception {ex}");
            }
        ...
        public static Texture2D LoadDDS(string path)
        {
            ...
            using (BinaryReader reader = new BinaryReader(File.Open(path, FileMode.Open, FileAccess.Read)))
            {
                ...
                texture.name = path.Substring(path.IndexOf("ROLoadingImages"));
